  ...
  <el-form-item label=" 监听端口" prop="desc2">
      ...
      <!--   <el-row><el-button  size="mini" icon="el-icon-circle-plus" circle @click="addport('add',0)">端口映射</el-button></el-row> -->
       ...
       <el-col :span="6">
        ...
        <el-col :span="6">
          <span class="padding-left10">{{data.launchConfig.ports[0]}}</span>
        </el-col>

        <el-col :span="6">
          <span class="padding-left10">协议 :http</span>
        </el-col>
...
import { linkmap,getlinkservice,getlb,getservice,hosts,addServer,addlinkcontainer,lb,addlinklb} from '@/api/paasApi'

  export default {
    name:'editlb',
    data() {
      return {
        lbid:this.$route.query.serviceId,
        linkmap:null,
        data:{
        ...
        getlinkservice(sid).then(response=>{
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                this.linkmap.findIndex(function(value, index, arr) {
                  if(value.serviceId===that.lbid && value.consumedServiceId==data[i].id){ 
                        console.log('map value',value)

                        linkname=value.name
                        let arrport= value.ports[0].split(":")
                        host=arrport[0]
                        pot=arrport[1].split("=")

                        console.log('value.ports[0]',value.ports)
                        console.log('arrport',arrport)
                        console.log('pot',pot)
                     }                  
                })
                this.link.push({'serviceId':data[i].id,'name':linkname,'hostname':host,'hostport':pot[0],'serviceport':pot[1]})
            }
            console.log(sarr)
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            lb(2,this.data,this.lbid).then(response=>{
